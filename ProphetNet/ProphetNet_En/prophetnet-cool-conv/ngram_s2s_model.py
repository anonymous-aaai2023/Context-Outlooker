...
class NgramTransformerProphetModel(FairseqEncoderDecoderModel):
    ...
    def add_args(parser):
        ...
        parser.add_argument('--outlooker-layers', type=int, metavar='N',
                            help='num outlooker layers')
    ...
    def build_model(cls, args, task):
        ...
        if args.load_from_pretrained_model is not None:
            ...
            #states = {k: v for k,v in states.items() if k in model_dict and v.size() == model_dict[k].size()}
            states = {k: v for k,v in model_dict.items() if k in states}
...
class TransformerEncoder(FairseqEncoder):
    ...
    def __init__(self, args, dictionary, embed_tokens, filter_sizes=[3,3,4,5]):
        ...
        self.outlooker_blocks = self.build_outlooker_blocks(Outlooker,embed_dim,2,
                                 downsample=False, num_heads=1,
                                 kernel_size=(3,embed_dim), stride=(2,embed_dim),
                                 padding=1, mlp_ratio=3,
    ...
    def forward(self, src_tokens, src_lengths, **unused):
        ...
        for layer in self.layers:
            ...
            x, _ = layer(x, self_attn_padding_mask=encoder_padding_mask,) #(src_len, batch, embed_dim)` ->  B, H, W, C

        x = x.permute(1,0,2).unsqueeze(1)

        pool = nn.AdaptiveMaxPool1d(x.size()[2])

        conved = [pool(F.relu(conv(x)).squeeze(3)) for conv in self.convs]
        conved = torch.cat(conved,1).permute(0,2,1).unsqueeze(-1)
        
        for layer in self.outlookers:
            conved = layer(conved)

        x = conved.squeeze(-1).permute(1,0,2)
